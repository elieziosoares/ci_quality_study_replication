...
con <- DBI::dbConnect(odbc::odbc(),
                      ...
                      UID      = "commitminer",
                      PWD      = "commitminer"#rstudioapi::askForPassword(prompt = "Please enter your password")
)
...
query_ci <- dbSendQuery(con, "SELECT CASE WHEN CI=TRUE THEN 1 WHEN CI=FALSE THEN 0 END CI FROM METRICS_PERIOD where PERIOD like 'MONTH' AND repo_name IN (select repo_name from projects where rq1_included is true) ORDER BY REPO_NAME,INIT_PERIOD")
ci <- data <- dbFetch(query_ci)
query_bugs <- dbSendQuery(con, "SELECT bugs FROM METRICS_PERIOD where PERIOD like 'MONTH'  AND repo_name IN (select repo_name from projects where rq1_included is true) ORDER BY REPO_NAME,INIT_PERIOD")
bugs <- data <- dbFetch(query_bugs)
query_commit_s <- dbSendQuery(con, "SELECT commit_size_mean commit_size FROM METRICS_PERIOD where PERIOD like 'MONTH'  AND repo_name IN (select repo_name from projects where rq1_included is true) ORDER BY REPO_NAME,INIT_PERIOD")
commit_s <- data <- dbFetch(query_commit_s)
query_merge_c <- dbSendQuery(con, "SELECT merge_conflicts Merge_Conf FROM METRICS_PERIOD where PERIOD like 'MONTH' AND repo_name IN (select repo_name from projects where rq1_included is true) ORDER BY REPO_NAME,INIT_PERIOD")
merge_c <- data <- dbFetch(query_merge_c)
query_communication <- dbSendQuery(con, "SELECT communication_mean Communication FROM METRICS_PERIOD where PERIOD like 'MONTH' AND repo_name IN (select repo_name from projects where rq1_included is true) ORDER BY REPO_NAME,INIT_PERIOD")
communication <- data <- dbFetch(query_communication)
query_test_v <- dbSendQuery(con, "SELECT test_volume_proportional test_vol FROM METRICS_PERIOD where PERIOD like 'MONTH' AND repo_name IN (select repo_name from projects where rq1_included is true) ORDER BY REPO_NAME,INIT_PERIOD")
test_v <- data <- dbFetch(query_test_v)
query_age <- dbSendQuery(con, "SELECT age FROM METRICS_PERIOD where PERIOD like 'MONTH' AND repo_name IN (select repo_name from projects where rq1_included is true) ORDER BY REPO_NAME,INIT_PERIOD")
age <- data <- dbFetch(query_age)
#query_issue_type <- dbSendQuery(con, "SELECT issue_type_mode issue_type FROM METRICS_PERIOD where PERIOD like 'MONTH'  AND builds_activity >0 ORDER BY REPO_NAME,INIT_PERIOD")
#issue_type <- data <- dbFetch(query_issue_type)
...
#####################################################
##Age ⊥ Commit_Size                                                                        #19.20786    #######               
dcov.test(df$age,df$commit_size,R=200)
##Age ⊥ Merge_Conflicts                                                                    #1.593328     #########
